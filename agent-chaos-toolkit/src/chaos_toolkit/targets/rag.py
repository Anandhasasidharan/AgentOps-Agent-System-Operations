...
def _simulate_corrupted_context(params: dict[str, Any]) -> dict[str, Any]:
    corrupted_content = params.get(
        "corrupted_content",
        "��\x00\x00\x00\x19t\x00h\x00i\x00s\x00 \x00i\x00s\x00 \x00c\x00o\x00r\x00r\x00u\x00p\x00t\x00e\x00d\x00 \x00d\x00a\x00t\x00a"
    )
    return {
        "fault_injected": True,
...
async def _simulate_slow_response(params: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "retrieved_chunks": [
            {"id": "chunk-1", "content": "This is a valid but very slow response.", "relevance": 0.85}
        ],
